...
export class Drafts implements OnInit {
  ...
  loadDrafts(): void {
    ...
    this.draftService.getDrafts()
      .pipe(
        finalize(() => {
          console.log('Draft API finished');
  ...
  copy(text: string): void {
    ...
    this.snackBar.open('Draft copied', 'Close', {
      duration: 2500
    });
  }
  ...
  approveDraft(id: string): void {
    this.draftService.approveDraft(id).subscribe({
      next: () => {
        this.snackBar.open('Draft approved', 'Close', {
          duration: 2500
        });
        this.loadDrafts();
      },
      error: (err) => {
        console.error(err);
        this.snackBar.open('Unable to approve draft', 'Close', {
          duration: 3000
        });
      }
    });
  ...
  sendDraft(id: string): void {
    this.draftService.sendDraft(id).subscribe({
      ...
      error: (err) => {
        console.error(err);
        this.snackBar.open(
          err?.error?.detail || 'Failed to send email.',
          'Close',
          { duration: 4000 }
        );
      }
  ...
  editDraft(draft: Draft): void {
    ...
    this.draftService.editDraft(draft.id, updated).subscribe({
      next: () => {
        this.snackBar.open('Draft updated', 'Close', {
          duration: 2500
        });
        this.loadDrafts();
      },
      error: (err) => {
        console.error(err);
        this.snackBar.open('Unable to update draft.', 'Close', {
          duration: 3000
        });
      }
    });
